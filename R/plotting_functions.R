...
theme_set(
  ...
  plot.title = element_text(family = "Helvetica Neue", size = 12, face = 'bold', hjust = .5),
  ...
  axis.title.x = element_markdown(family = "Helvetica Neue", hjust = .5, size = 12, color = "grey40"),
  axis.title.y = element_markdown(family = "Helvetica Neue", hjust = .5, size = 12, color = "grey40"),
  axis.text = element_text(family = "Helvetica Neue", hjust = .5, size = 12, color = "grey40"),
  legend.position = "top",
...
plot_prevalencebyquantile <- function(data_prs){
  ...
  data2plot |>
    ...
    annotate(
      geom = 'richtext',
      x = 90,
      label = str_c("<span style='font-size:12pt'>at 90th</span><br>",data2plot |> filter(percentile == 90) |> pull(prevalence) |> percent()),
      y = .5,
      vjust = 1,
      size = 6,
      fill = NA,
    ...
    annotate(
      geom = 'richtext',
      x = 10,
      label = str_c("<span style='font-size:12pt'>at 10th</span><br>",data2plot |> filter(percentile == 10) |> pull(prevalence) |> percent()),
      y = .5,
      vjust = 0,
      size = 6,
      fill = NA,
...
plot_roc <- function(data_prs){
  ...
  ggroc(roc_obj,  color = palette['case']) +
    ...
    annotate(
      "richtext", 
      size = 6,x = 0.5, y = 0.4, 
      label = paste("<span style='font-size:12pt'>AUC</span><br>", 
      round(auc_value, 3)), 
      vjust = 1, 
      hjust = 0,
      fill = NA,
...
plot_ors <- function(data_prs){
  ...
  regression_data <- data2plot |>
    ...
    mutate(
      ...
      prs_quintile_factor = factor(prs_quintile, levels = c("40-60%", "0-20%", "20-40%", "60-80%", "80-100%"))
    )

  model <- glm(outcome ~ prs_quintile_factor, data = regression_data, family = binomial(link = "logit"))
  ...
  or_results <- as_tibble(or_ci, rownames = "term") |>
    ...
    mutate(prs_quintile = factor(term, levels = c("0-20%", "20-40%", "40-60%", "60-80%", "80-100%"))) |>
    arrange(prs_quintile) |>
